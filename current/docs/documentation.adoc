...
The user guide needs to be fully descriptive and easy to read, including not only technical documentation but use cases and error solutions. The documentation should be written using asciidoc syntax and the resulting file called user_manual.adoc. A template can be found link:attachments/user-manual.adoc[here].


=== Release Notes

Release notes need to be highly descriptive, specially including backward compatibility of the released connector with previous versions. A template can be found link:attachments/releaseNotesTemplate.adoc[here].
